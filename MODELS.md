...
### Live test: Qwen3.5 4B vs Qwen3 4B on a real rename, 16GB machine

Ran the same `rename_symbol` task (rename a PHP class, moving its file) against both. Qwen3.5 4B produced the correct result — the right file rename, the right patches, a clear and accurate explanation — matching its strong benchmark reputation. But it took noticeably longer end to end (several minutes vs under a minute for `qwen3:4b`/`nativestudio:coder` on the same machine) and held closer to 9GB of VRAM at idle rather than qwen3:4b's ~7.5GB, almost certainly from Qwen3.5's much larger native context window (262K tokens) carrying real KV-cache overhead even for a short conversation. On a RAM-constrained machine, the "better tool-calling" model isn't automatically the better choice in practice — the benchmark table measures correctness, not what your specific hardware does with it.
